...
| 56 | TikTok                             | *.tiktokcdn.com (signed)      | **collect (network-free)** — see row 116 (page-JSON reader)                                | **L**   |
...
| 116 | TikTok | tiktok.com (video/photo page) → v*-webapp/tiktokcdn(-us).com | **collect (network-free)** — a video-detail page embeds the item in `<script id="__UNIVERSAL_DATA_FOR_REHYDRATION__">` under `__DEFAULT_SCOPE__["webapp.video-detail"].itemInfo.itemStruct`. collect.ts reads the URLs TikTok itself signed (passive, like IG/FB) — the highest-`Bitrate` rendition's `PlayAddr.UrlList[0]` (else `playAddr`) as one mp4, poster from `cover`; a photo-mode `imagePost` → one image per slide. Host-pinned to the TikTok CDN allowlist. Private/removed → no itemStruct → [] (fails closed). Edges are session/hotlink-bound → download via #197 Referer + browser cookies. Structure well-documented; automation is bot-walled (403), so **needs-live-confirmation** | L |
| 117 | Twitch (VODs) | twitch.tv/videos/<id> → usher.ttvnw.net (HLS) | **resolver** (opt-in **N**) — a VOD has no single mp4, so an anonymous `PlaybackAccessToken` GQL query (raw, not persisted; public web Client-ID) mints the sig+token that authorizes `usher.ttvnw.net/vod/<id>.m3u8`, the HLS master, returned to capture and host-pinned to `ttvnw.net`. Complements the clip resolver (row 64-family). Sub-only/private VODs mint a token usher then 403s → capture fails (no circumvention) | N |
| 118 | SoundCloud | soundcloud.com (track) → *.sndcdn.com (HLS/progressive) | **resolver** (opt-in **N**, audio) — needs an anonymous `client_id` (scraped from the page's `a-v2.sndcdn.com` bundle) for the public `api-v2` endpoints: page → client_id → `resolve` → track JSON → a transcoding → the CDN stream. Prefers the HLS rendition (the capture engine turns an audio-only master into m4a/mp3 via the existing audio path, honouring the MP3-transcode setting), else a progressive single file. Track URL pinned to `soundcloud.com`, final stream to `sndcdn.com`. Non-track (user/playlist) or private/go+ → null | N |
| 119 | Patreon | patreon.com (post) → *.patreonusercontent.com | **collect (network-free)** — a post's images ship at multiple sizes in the hydrated markup under `…/patreon-media/p/post/<postId>/<hash>/<transform>/<file>`; collect.ts scrapes them scoped to the URL's `<postId>` (campaign art `/p/campaign/…` and other posts' feed media excluded), groups renditions per `<hash>/<file>`, and keeps the largest — the un-resized original (transform decodes to `{"a":1,…}`) when shipped, else the widest `w`/`h` — with its signed query intact (token is rendition-bound; free-ride). Public posts work; a paid/locked post the viewer can't access ships none → [] (fails closed). Token-gated → download via #197 Referer. Images/GIFs only (video/audio attachments a follow-up); URL shapes verified against real samples, **needs-live-confirmation** on a post page | L |
| 120 | Kemono / Coomer | {kemono,coomer}.{cr,su,st,party} (post) → &lt;host&gt;/data | **collect (network-free)** — a Patreon/Fanbox/etc. mirror; a post (`/<service>/user/<id>/post/<postId>`) server-renders its files/attachments as `<host>/data/<hash>.<ext>?f=<name>` links (public paths, no token). collect.ts surfaces this post's originals (image/gif/video), deriving the ext from the path or the `?f=` filename, skipping the `/thumbnail/` preview server and any off-host URL. A post the viewer can't access renders no `/data/` links → [] (fails closed). Host-gated to the known TLDs. Endpoint/URL shapes from gallery-dl; **needs-live-confirmation** (host is DDoS-Guard-fronted) | L |
| 121 | Erome | erome.com (album `/a/<id>`) → *.erome.com | **collect (network-free)** — an album page renders each item in a `<div class="media-group">`: a video ships a `<video><source src>`, an image a lazy `<img data-src>` (the visible src is a placeholder). collect.ts reads those CDN URLs directly (host-pinned to `*.erome.com`), one candidate per group, deduped. A private/removed album renders no media-groups → [] (fails closed). URL/DOM shapes from gallery-dl; **needs-live-confirmation** | L |
| 122 | Image Chest | imgchest.com (post `/p/<id>`) → cdn.imgchest.com | **collect (network-free)** — the Inertia app serializes the whole post into the root element's `data-page` attribute, so every file's original CDN URL (`cdn.imgchest.com/files/<id>.<ext>`) is already in the markup; collect.ts scans them out (images/GIFs + mp4), deduped, ordered by appearance. A private/empty post ships no file URLs → [] (fails closed). URL/attribute shapes from gallery-dl; **needs-live-confirmation** | L |
| 123 | Fapello | fapello.com/.su (post `/<model>/<id>/`) | **collect (network-free)** — a post page renders one media item in a `uk-align-center` block: an `<img src>` (with the `.md`/`.th` thumbnail size suffix stripped → the original) or a video (`type="video"`/`<video>`, `poster` kept). collect.ts reads just that block, host-gated, excluding category/listing first-segments (`trending`/`videos`/…). A listing or an inaccessible post yields no block → [] (fails closed). Structure from gallery-dl; automation bot-walled (403) → **needs-live-confirmation** | L |
| 124 | Chevereto | jpgfish (jpg*.{cr,su,pet,fish,church}) / imglike.com / putme(ga).* (image `/img|image|i/<id>`) | **collect (network-free)** — Chevereto image-host software; the viewer page's `og:image` is the full-resolution original. collect.ts reads it **only** when it is a plaintext `https` media URL — jpgfish instances that ship an XOR-encrypted or `loading.svg`-placeholder `og:image` are skipped, not decrypted (fails closed, no circumvention). Host-gated to the gallery-dl instance list (domains churn). One candidate/page; **needs-live-confirmation** | L |
| 125 | Imgur | imgur.com (post/`/a/`/`/gallery/`) → i.imgur.com | **collect (network-free)** — the page assigns the whole post to `window.postDataJSON` (a JS string literal); collect.ts parses it and surfaces each `media[].url` original on i.imgur.com (an album ships every item — covering media the DOM lazy-loads), pinned to the CDN and classified by ext (gif/mp4/image). A removed/empty post → [] (fails closed). Verified live (`media[0].url` present) | L |
| 126 | Tenor | tenor.com (view `/view/<slug>-<id>`) → media*.tenor.com | **collect (network-free)** — the view page embeds `<script id="store-cache">`; `gifs.byId[<id>].media_formats` carries the renditions. collect.ts surfaces the animated `gif` (else the muxed `mp4`/`webm`), pinned to media*.tenor.com. No cached entry for the URL's id → [] (fails closed). Verified live | L |
| 127 | Pexels | pexels.com (photo/video) → images/videos.pexels.com | **collect (network-free)** — the page embeds `<script id="__NEXT_DATA__">`; `props.pageProps.medium.{video,image}.download_link` is the free full-resolution original. collect.ts reads it **same-origin in the content script** (the page is Cloudflare-gated → a background fetch 403s, the Kick/Shopify lesson), pinned to the Pexels CDN; the existing images.pexels.com rule then strips its query to the bare original. No `medium` → [] (fails closed) | L |
| 128 | Civitai | image.civitai.com (CDN) | **CDN rule** — `image.civitai.com/<bucket>/<uuid>/<transform>/<name>`: the transform segment before the filename (`width=450`, `anim=false,width=…`) is rewritten to `original=true` for the un-resized original (host-pinned; bucket-agnostic). A URL already at `original=true` or with no `=` transform segment is left untouched. Deterministic, no probe | C |
