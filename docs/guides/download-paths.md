...
- **Safety** — the whole expanded path is run through `sanitizePathSegment`
  (`src/extension/shared/paths.ts`): traversal (`..`), leading slashes, illegal
...
- `buildDownloadFilename(image, index, settings, sourcePageUrl?)` — resolves the
  ...
  (`src/extension/background.ts`); the source URL is threaded from
  `downloadAndRecord`.
