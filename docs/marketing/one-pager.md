...
- **Bulk or precise.** Download a single item, a filtered set (by type, format, dimensions, or size), or the whole page. The download queue handles large batches and resumes cleanly.
...
Collecting media reads the page's DOM and rewrites URL strings. It makes no background requests and phones home to no one. Any network use (e.g. fetching a true original from a public API) is opt-in and off until you enable it.
