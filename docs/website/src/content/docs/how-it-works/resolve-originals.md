...
| `kick`         | *(clip)* `https://kick.com/api/v2/clips/<id>/play`. *(VOD, hint = `video <uuid>`)* `https://kick.com/api/v1/video/<uuid>`                                                                                                                                                                    | Near-1:1 with the Twitch resolver. A clip returns its progressive mp4 (`clip.clip_url`, else `clip.video_url`, served from `clips.kick.com`); a VOD returns the `source` HLS master (from `stream.kick.com`) to capture. Both media URLs are host-pinned to `kick.com`; the API JSON is untrusted. Private/expired media and live channels resolve to `null` (no circumvention).                                                                                                                                                                                                                            |
...
`ttvnw.net` (Twitch VOD master), `kick.com` (Kick clips + VOD), `sndcdn.com` (SoundCloud stream),
