...
describe('resolve — generic fallback', () => {
  ...
  it('routes a v.sankakucomplex.com original through the sankaku resolver with an md5 mediaKey', () => {
    ...
    expect(c).toMatchObject({ kind: 'image', url, mediaKey: 'sankaku 2620d86cb72802a5dcd9e1e189b75e64' });
  });

  it('includes xiaohongshuResolver before genericResolver', () => {
    const ids = REGISTRY.map((r) => r.id);
  ...
  it('routes a signed RED note-image URL through the xiaohongshu resolver with a fileId mediaKey', () => {
    ...
    const [c] = resolve(url, ctx);
    expect(c).toMatchObject({ kind: 'image', mediaKey: 'xhs notes_pre_post/1040g3k8321i4pbs37k7g5o5dgbqgbkc6gdrpq90' });
  });
});
