...
test.describe('realistic sites', () => {
  test('X/Twitter: collapses photo sizes, surfaces native-video + gif as Video items, excludes the pbs host', async ({ context }) => {
    ...
    await expect(figureWithSrc(page, 'GAAA111PhotoAA')).toHaveCount(1);
  ...
  test('Generic web: collects srcset / picture / lazy / background / og / gallery media', async ({ context }) => {
    ...
    await openPanel(page);
    await expect(figureWithSrc(page, 'og-hero')).toHaveCount(1);
    await expect(figureWithSrc(page, 'large.jpg')).toHaveCount(1);
  ...
  test('Pinterest: collapses /<size>/ variants to one original + surfaces the video pin, then excludes the pinimg host', async ({ context }) => {
    ...
    await expect(figureWithSrc(page, '45791643dd397b203c0306f076d94e0b')).toHaveCount(1);
    await expect(figureWithSrc(page, 'BBBB1111222233334444555566667777')).toHaveCount(1);
  ...
  test('Reddit: collapses preview.redd.it renditions to the i.redd.it original; leaves external-preview alone', async ({ context }) => {
    ...
    await openPanel(page);
    expect(await itemCount(page)).toBe(3);
    await expect(figureWithSrc(page, 'ch5ejccb04ch1')).toHaveCount(1);
    await expect(figureWithSrc(page, 'abcd1234efgh5678')).toHaveCount(1);
    await expect(figureWithSrc(page, 'external-preview.redd.it')).toHaveCount(1);
  });
  ...
  test('Flickr: collapses sub-_b sizes to _b, keeps the different-secret _6k, passes the buddyicon through', async ({ context }) => {
    ...
    await openPanel(page);
    expect(await itemCount(page)).toBe(3);
    await expect(figureWithSrc(page, '55379291849')).toHaveCount(1);
    await expect(figureWithSrc(page, '99887766554_3d3e638f8b_6k')).toHaveCount(1);
    await expect(figureWithSrc(page, 'buddyicon')).toHaveCount(1);
  });
  ...
  test('ArtStation: collapses /small/ + /medium/ to /large/ and surfaces the video-clip poster as a Video item', async ({ context }) => {
    ...
    await openPanel(page);
    expect(await itemCount(page)).toBe(3);
    await expect(figureWithSrc(page, 'ed-pantera-ts01')).toHaveCount(1);
