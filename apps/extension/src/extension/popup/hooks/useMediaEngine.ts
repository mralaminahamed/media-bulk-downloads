...
import { AppState, DeepScanProgress, DeepScanStopReason, FilterOptions, ImageInfo, OriginalCaptureProgress, SettingsData } from '@mbd/core/types';
...
export interface UseMediaEngineParams {
  ...
  abortDeepScan: () => void;
  /** How to run Facebook original-capture from the popup. Absent → the capture control is hidden. */
...
export interface UseMediaEngineResult {
  ...
  /** True while a Facebook original-capture run is in flight. */
  capturing: boolean;
  captureProgress: OriginalCaptureProgress | null;
  /** True while the inline "open N photos?" confirm is showing. */
  confirmCapture: boolean;
  setConfirmCapture: Dispatch<SetStateAction<boolean>>;
  /** Toggle the capture flow (show the confirm, or abort an in-flight run). */
  handleCaptureOriginals: () => void;
  /** Run the capture after the confirm is accepted. */
...
export function useMediaEngine({
   ...
   */
  const applyFreshSnapshot = (found: ImageInfo[], s: SettingsData): void => {
    if (!found.length) {
      setState((prev) => ({ ...prev, status: 'No originals captured.' }));
      return;
    }
    rawImagesRef.current = found;
    const eligible = filterExcluded(filterImagesBySettings(found, s), excludedRef.current);
    applyResolution(eligible, s);
  };
  ...
  const runCapture = async (): Promise<void> => {
    ...
    try {
      ...
      // would leave both the low-res and full-res rows. `found` is a complete fresh
      // collectMedia() snapshot of the page (all tiles, upgraded) — replace via the
      // shared helper (which also guards an empty/failed capture from wiping the grid).
      applyFreshSnapshot(found, settingsRef.current);
    } catch (e) {
  ...
  const handleDeepScan = async () => {
    ...
    try {
      ...
      if (captureOriginals && latest.fbCaptureOriginals) {
        // On a FB photo grid with the master toggle on, the content script chained an
        // original-capture pass after its scroll loop and re-collected, so `found` is a
        // COMPLETE upgraded snapshot (every tile, upgraded to a full-res URL) — replace,
        // don't merge, or the stale low-res row and the upgraded row (different canonical
        // keys) would both survive.
        applyFreshSnapshot(found, latest);
