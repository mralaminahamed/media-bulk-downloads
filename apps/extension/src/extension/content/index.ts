...
import { SettingsData, DeepScanProgress, OriginalCaptureProgress, SettingsChangedMessage } from '@mbd/core/types';
...
import { startOriginalCapture, runCaptureOnLoadedTiles } from '@/extension/content/originalCaptureRunner';
...
export function shouldChainCapture(scanHost: string, s: SettingsData): boolean {
  const onFb = scanHost === 'facebook.com' || scanHost.endsWith('.facebook.com');
...
// Registered third (after GET_IMAGES and DEEP_SCAN) so the popup-driven capture
// path — "Fetch full-res originals" — can start/abort a run and stream progress,
// independent of the deep-scan → capture chain above.
