...
- **Dashboard window**: a React app (`dashboard/`, built by Vite → embedded via
  ...
  under the header. It is served by `Deno.serve` on `127.0.0.1:<random>` and talks
  to the backend over `fetch('/api/…')` plus an SSE `/events` stream. A per-session
  token (minted at startup and embedded in the served HTML shell, since the webview
  drops the launch URL's query string) guards every `/api`/`/events` request. It
  shows the media grid (with a filter toolbar for kind/format/size/search/sort,
  reusing `@mbd/core`'s filter predicates), multi-select, preview,
  download-to-queue, live queue status, History/Favourites tabs, and a Settings
  surface (Downloads/Media/Display/Data/Advanced panes on KV, including backup
  ...
  "Capture" action: the Deno backend fetches, decrypts, and muxes the stream to an
...
is Deno KV (`src/storage/*`); the download queue and downloader are `src/platform/*`.
...
Reuses `@mbd/core` from the monorepo. Runtime gotchas and the two-window model are
