...
> **Safari** is a work-in-progress build target (`yarn build:safari` →
> `apps/extension/.output/safari-mv3`). The extension code targets Safari through
> the `@mbd/platform` capability seam, but it is **not yet shipped**: the
> background isn't fully routed through the seam, and the native macOS wrapper
> (`apps/safari-native/`, generated by `safari-web-extension-converter`) plus App
> Store submission require macOS + Xcode. See [#307](https://github.com/mralaminahamed/media-bulk-downloads/issues/307)
> and [`docs/store-submissions/SAFARI_APPSTORE.md`](./docs/store-submissions/SAFARI_APPSTORE.md).
